...
class Sapphire():
    ...
    def __setup_connection(self) -> None:
        ...
        if api_key is None:
            print('Make sure to add your OpenAI API KEY to your system environment as OPENAI_API_KEY.')
            sys.exit(0)
    ...
    def __detect_system(self) -> None:
        ...
        if platform.startswith('linux'):
            ...
            print('User platform is incompatible with Sapphire :(')
            sys.exit(0)
    ...
    def __greenlight_sapphire_cmd(self, sapphire_cmd) -> bool:
        print(f'Executing {sapphire_cmd}... [Y/N]')
        res = input()
        if res == 'Y' or res == 'y':
            return True 
        return False
